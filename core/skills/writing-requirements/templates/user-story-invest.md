...
| Small | Fits inside a normal delivery iteration. | Split if the story combines multiple goals, workflows, or **Acceptance Criteria describing behavior of a different persona**. |
...
- **Single-Persona Rule:** Every AC and Scenario must describe behavior observable from the persona in the Story Statement. If another persona's behavior is relevant, express it as a `Given` precondition or split it into a separate User Story.
- Keep implementation details out of every section of the story (statement, Acceptance Criteria, Scenarios). If the user mentioned specific UI components, services, or technology during discussion, convert them to behavioral descriptions in the artifact. See `templates/user-story-quality-checklist.md` → **Negotiable Blocklist** for a concrete keyword reference.
